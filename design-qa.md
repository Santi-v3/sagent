...
- **State:** Pending plan with full TaskPlanner, risks, ChangeProposal and approval actions. Approved, needs-changes, successful-test and failed-test states were checked separately.
...
- **Interaction and accessibility:** Plan creation, keyboard submission, all three approval decisions, test-profile selection, test execution, result logs, disabled/loading states and retry behavior are functional and labeled.
- **Responsiveness:** At 390 × 844 the sidebar collapses, document width remains equal to viewport width, controls stack to full width and test logs remain readable without horizontal page overflow.
...
- Verified real successful and intentionally failing test processes in the browser; both preserve the exact command, exit code and bounded logs.
- Verified the test-result layout on desktop and at 390 × 844 with no browser console errors.
